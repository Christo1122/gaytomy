...
int main(void)
{
    ...
    // Load bunny texture
    Texture2D texBunny = LoadTexture("resources/tomy1.png");
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            DrawText(TextFormat("gaytomies: %i", bunniesCount), 120, 10, 20, GREEN);
            DrawText(TextFormat("batched draw calls: %i", 1 + bunniesCount/MAX_BATCH_ELEMENTS), 320, 10, 20, MAROON);
            DrawText("gaytomy ",10,60,100,BLACK);
            DrawText("touch anyuwhere on the screen",140,200,26,GREEN);
